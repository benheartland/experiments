...
function playNote(instrumentInstance, noteNumber, velocity) {
	var sp = instrumentInstance.note[noteNumber].samplePointer;
	// set the volume using a simple linear velocity curve.
	instrumentInstance.note[noteNumber].sample[sp].volume = velocity/127;
	// if the sample is not playing, start it...
	if (instrumentInstance.note[noteNumber].sample[sp].ended || instrumentInstance.note[noteNumber].sample[sp].paused) {
		instrumentInstance.note[noteNumber].sample[sp].play();
	// ...otherwise return it to it's start position.
	} else {
		instrumentInstance.note[noteNumber].sample[sp].currentTime = 0;
	}
	...
	instrumentInstance.note[noteNumber].samplePointer++;
	if (instrumentInstance.note[noteNumber].samplePointer >= instrumentInstance.note[noteNumber].sample.length) {
		instrumentInstance.note[noteNumber].samplePointer = 0;
	}
}

function stopNote(instrumentInstance, noteNumber) {
	for (var i = 0; i++; i < instrumentInstance.note[noteNumber].sample.length) {
		if (!instrumentInstance.note[noteNumber].sample[i].paused) {
			instrumentInstance.note[noteNumber].sample[i].pause();
		}
		instrumentInstance.note[noteNumber].sample[i].currentTime = 0;
	}
}
